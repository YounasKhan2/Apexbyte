...
const out180 = resolve(process.cwd(), 'public/icons/icon-180.png');
...
async function main() {
  ...
  await ensureDir(out180);
  await ensureDir(out256);
  await ensureDir(out384);
  await ensureDir(out512);
  const img180 = await sharp(svg).resize(180, 180).png().toBuffer();
