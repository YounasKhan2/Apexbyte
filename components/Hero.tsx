...
export default function Hero() {
  ...
  <div className="pointer-events-none absolute -top-24 -right-24 h-[520px] w-[520px] rounded-full bg-gradient-to-br from-primary/20 to-purple/20 blur-3xl" />
        <svg className="absolute left-1/2 top-1/2 -z-10 -translate-x-1/2 -translate-y-1/2 opacity-10" width="700" height="700" viewBox="0 0 700 700" fill="none">
          <defs>
      ...
      <div className="container grid gap-10 md:grid-cols-2 items-center">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          <h1 className="font-heading text-4xl md:text-6xl leading-tight tracking-tight">
            We craft premium
            <span className="ml-3 rounded-2xl bg-slate-100 px-3 py-1 text-primary shadow-inner">{display}</span>
          </h1>
          ...
          <div className="mt-8 flex gap-4">
            <a href="#contact" className="rounded-2xl bg-primary text-white px-5 py-3 shadow-glow hover:opacity-95">Start a project</a>
            <a href="#portfolio" className="rounded-2xl border border-slate-200 px-5 py-3 hover:shadow-soft">See our work</a>
          </div>
        </motion.div>
